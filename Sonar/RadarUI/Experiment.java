...
public class Experiment extends PApplet
{
  ...
  void drawObject()
  {
    if (this.buffer.wasModified()) {
      ...
      {
        // angle = radians(angle);
        // ellipse((float)(width-width*0.505)*cos(radians(angle)), (float)-(width-width*0.505)*sin(radians(angle)), 5, 5);
        float x = map(distance*cos(radians(angle)),0,200,0,width/2);
        float y = -map(distance*sin(radians(angle)),0,200,0,width/2);
        ellipse(x, y, 4, 4);
        // line(pixsDistance*cos(radians(angle)),-pixsDistance*sin(radians(angle)),(float)(width-width*0.505)*cos(radians(angle)),(float)-(width-width*0.505)*sin(radians(angle)));
      }
      popMatrix();
    }
  }
